...
and hence since $P(0)$ is True and (B1) says $P(0)\rightarrow P(1)$, we know $P(1)$ is True.
...
and if $n=0$, then $n(n+1)(2n+1)/6= (0 * 1 * 1)/6 =0$
...
$$(\sum_\limits{i=0}^{k-1} i^2) + k^2 = (2k^3-3k^2 +k)/6 +k^2 = (2k^3-3k^2 +k)/6 +(6k^2/6) = (2k^3+3k^2 +k)/6 
= k(k+1)(2k+1)/6$$
...
**Proof:**
In the base case, where $n=0$, there are no operators, the formula must be a propositional symbol and hence have just 1 node and $2n+1 = 2 * 0 + 1 = 1. So it is True in the base case.

In the induction case, where $n>1$ there is a main operator at the top of the parse tree and it connects two smaller formulas, say $F_1$ and $F_2$, e.g.  $F_1\vee F_2$.  By induction, if $F_i$ has $n_i$ operators, and $n_i<n$, $F_i$ must have $2n_i+1$ nodes. Since $n = 1 + n_1 + n_2$, we see that the number of nodes in the original formula is $1 + 2n_1+1 + 2n_2+1 = 2(n_1+n+2+1) + 1 = 2n+1$ as was to be proved. **Q.E.D.**
