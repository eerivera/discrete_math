...
**Answer:**
We want to simplify the following formula:

$\forall x \forall y ( P(x,y) \rightarrow \exists x Q(x,y) ) $ 

First we rename the $\exists x$ variable

$\forall x \forall y ( P(x,y) \rightarrow \exists z Q(z,y) ) $

Then we replace the implication with a $\vee$

$\forall x \forall y ( \neg P(x,y) \vee \exists z Q(z,y))$

Then we introduce a skolem function f(x,y) for z

$\forall x \forall y ( \neg P(x,y) \vee Q(f(x,y),y))$

and we are done since the quantifiers are already at the front
