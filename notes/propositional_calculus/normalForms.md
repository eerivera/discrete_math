...
A disjunction of literals is called a "clause." We can write such a clause by putting the
negative literals first and the positive literals second:

$\neg A \vee \neg B \vee \ldots \vee \neg C \vee D \vee E \vee \ldots F$

and by applying DeMorgan's rule we get an if-then rule:
